...
class GeneratorBlock(nn.Module):
    def __init__(self, latent_dim, input_channels, filters, upsample=True, upsample_rgb=True, rgba=False, cond_dim=1024):
        super().__init__()
        ...
        self.cond_trans1 = nn.Linear(cond_dim, input_channels)
        self.MHCA1 = MultiHeadCrossAttention(d_model=input_channels, num_heads=input_channels/32)
        self.conv1 = Conv2DMod(input_channels, filters, 3)

        self.to_style2 = nn.Linear(latent_dim, filters)
        self.to_noise2 = nn.Linear(1, filters)
        self.cond_trans2 = nn.Linear(cond_dim, filters)
        self.MHCA2 = MultiHeadCrossAttention(d_model=filters, num_heads=filters/32)
    ...
    def forward(self, x, prev_rgb, istyle, inoise, condition):
        ...
        condition_trans1 = self.cond_trans1(condition)
        style1 = self.MHCA1(style1, condition_trans1)
        x = self.conv1(x, style1)
        x = self.activation(x + noise1)

        style2 = self.to_style2(istyle)
        condition_trans2 = self.cond_trans2(condition)
        style2 = self.MHCA2(style2, condition_trans2)
...
class Generator(nn.Module):
    def __init__(self, image_size, latent_dim, network_capacity=16, transparent=False, attn_layers=[], no_const=False,
        ...
        #filters = [i * 2 for i in filters]  # MODIFIED
    ...
    def forward(self, styles, input_noise, condition):
        ...
        for style, block, attn in zip(styles, self.blocks, self.attns):
            if exists(attn):
                x = attn(x)
            x, rgb = block(x, rgb, style, input_noise, condition)
...
class Discriminator(nn.Module):
    def __init__(self, image_size, network_capacity=16, fq_layers=[], fq_dict_size=256, attn_layers=[],
        ...
        blocks = []
        #filters = [num_init_filters] + [(network_capacity * 2) * (2 ** i) for i in range(num_layers + 1)]  # MODIFIED *2
        filters = [num_init_filters] + [int(network_capacity/2) * (2 ** i) for i in range(num_layers + 1)]  # MODIFIED *2
...
class StyleGAN2(nn.Module):
    def __init__(self, image_size, latent_dim=2048, fmap_max=1024, style_depth=8, network_capacity=16, transparent=False,
