...
from models.GenSID_2dg import *
from utils.loss_2dg import *
...
class Trainer():
    def __init__(
        ...
        #self.classifier.load_state_dict(torch.load('/data/zyli/models/Project2/2D_StyleGAN_RGB/Classifiers/resnet18_re256_try1_E3_acc9193.pkl'))
        #empty = torch.nn.Sequential()
        #self.classifier.fc = empty
        #self.classifier = self.classifier.cuda(rank)
        #self.classifier.eval()
        self.classifier = timm.create_model('swinv2_base_window8_256', pretrained=True)
        patch_embed = self.classifier.patch_embed
        original_proj = patch_embed.proj
        new_proj = nn.Conv2d(1, 128, kernel_size=(4, 4), stride=(4, 4))
        patch_embed.proj = new_proj
        original_head = self.classifier.head
        in_features = original_head.fc.in_features
        self.classifier.head.fc = torch.nn.Linear(in_features, 2)
        #self.classifier.load_state_dict(torch.load(
        #    '/data/zyli/models/Project2/2D_StyleGAN_Xray/Classifiers_new/swinv2_base_window8_256_lr2e-5cosdecaytry1_Epoch7_acc9022.pkl'))
        self.classifier.load_state_dict(torch.load(
            '/data/zyli/models/Project2/2D_StyleGAN_Xray/Classifiers_new/swinv2_base_window8_256_lr5e-5cosdecayaug_try1_Epoch8_acc9108.pkl'))
        empty = torch.nn.Sequential()
        self.classifier.head.drop = empty
        self.classifier.head.fc = empty
        self.classifier.head.flatten = empty
        self.classifier = self.classifier.cuda(rank)
        self.classifier.eval()
        # 冻结模型的参数
        ...
        #self.cls_ori.load_state_dict(torch.load('/data/zyli/models/Project2/2D_StyleGAN_RGB/Classifiers/resnet18_re256_try1_E3_acc9193.pkl'))
        #self.cls_ori = self.cls_ori.cuda(rank)
        #self.cls_ori.eval()
        self.cls_ori = timm.create_model('swinv2_base_window8_256', pretrained=True)
        patch_embed = self.cls_ori.patch_embed
        original_proj = patch_embed.proj
        new_proj = nn.Conv2d(1, 128, kernel_size=(4, 4), stride=(4, 4))
        patch_embed.proj = new_proj
        original_head = self.cls_ori.head
        in_features = original_head.fc.in_features
        self.cls_ori.head.fc = torch.nn.Linear(in_features, 2)
        #self.cls_ori.load_state_dict(torch.load(
        #    '/data/zyli/models/Project2/2D_StyleGAN_Xray/Classifiers_new/swinv2_base_window8_256_lr2e-5cosdecaytry1_Epoch7_acc9022.pkl'))
        self.cls_ori.load_state_dict(torch.load(
            '/data/zyli/models/Project2/2D_StyleGAN_Xray/Classifiers_new/swinv2_base_window8_256_lr5e-5cosdecayaug_try1_Epoch8_acc9108.pkl'))
        self.cls_ori = self.cls_ori.cuda(rank)
    ...
    def init_GAN(self):
        ...
        # warm-up步数
        warmup_iters = 4000  # warm-up的迭代次数
        # 定义学习率调度器
        self.D_scheduler = WarmUpCosineLR(self.GAN.D_opt, total_iters=100000, warmup_iters=warmup_iters, min_lr=1e-8)
        self.G_scheduler = WarmUpCosineLR(self.GAN.G_opt, total_iters=100000, warmup_iters=warmup_iters, min_lr=1e-8)
        self.E_scheduler = WarmUpCosineLR(self.E_opt, total_iters=100000, warmup_iters=warmup_iters, min_lr=1e-8)
    ...
    def load_config(self):
        ...
        self.fmap_max = config.pop('fmap_max', 1024)
    ...
    def set_data_src(self):
        ...
        self.dataset = ChestXRayDataset(mode='labeled', fold=4)
    ...
    def train(self):
        ...
        apply_wc_loss = self.steps > 30000
        apply_cls_loss = self.steps > 30000
        ...
        we = we.reshape(we.shape[0], 2048, -1).mean(dim=-1)
        w = [(we, num_layers)]
        w = styles_def_to_tensor(w)
        wc = self.classifier(image_batch)
        wc = wc.unsqueeze(1)  # B, 1, 1024
        ...
        self.D_scheduler.step()  # MODIFIED
        self.tmp_lr_D = self.D_scheduler.get_lr()[0]
        ...
        if self.steps <= 30000:
            gen_loss = gen_loss + 10*L1loss + 4*Perceptualloss + 2*MSSSIMloss
        elif 30000 < self.steps <= 50000:
            gen_loss = gen_loss + 10*L1loss + 4*Perceptualloss + 2*MSSSIMloss + 3*wcLoss + 3*clsLoss
        ...
        self.tmp_lr_G = self.G_scheduler.get_lr()[0]
        self.tmp_lr_E = self.E_scheduler.get_lr()[0]
    ...
    def evaluate(self, num=0, trunc=1.0):
        ...
        with torch.no_grad():
            ...
            eva_loader = ChestXRayDataset(mode="labeled_test", fold=4)  # MODIFIED
            eva_cond = []
            eva_we = []
            for i in range(num_rows ** 2):
                ...
                we = self.encoder(img)
                we = we.reshape(2048, -1).mean(dim=-1)  # (B, 4096)
    ...
    def print_wb(self, wbrun):
        wbrun.log({"G_loss": self.g_loss, "D_loss": self.d_loss,
                   ...
                   "lr_D": self.tmp_lr_D, "lr_G": self.tmp_lr_G, "lr_E": self.tmp_lr_E
         })
